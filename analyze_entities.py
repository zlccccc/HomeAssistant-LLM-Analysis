...
def get_entity_info(entity_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"获取实体 {entity_id} 信息失败: {response.status_code}")
            return None
    except Exception as e:
        print(f"获取实体 {entity_id} 信息异常: {str(e)}")
        return None
...
def get_entity_history(entity_id: str, hours: int = 24) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"获取实体 {entity_id} 历史数据失败: {response.status_code}")
            return None
    except Exception as e:
        print(f"获取实体 {entity_id} 历史数据异常: {str(e)}")
        return None
...
def get_all_entities() -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"获取所有实体失败: {response.status_code}")
            return None
    except Exception as e:
        print(f"获取所有实体异常: {str(e)}")
        return None
...
def main() -> None:
    ...
    if sensor_data:
        print("\n传感器:")
        print(f"- 数值型传感器: {len(sensor_data.get('numeric_sensors', []))}")
        print(f"- 文本型传感器: {len(sensor_data.get('text_sensors', []))}")
        print(f"- 无效传感器: {len(sensor_data.get('invalid_sensors', []))}")
    ...
    if summary_file and analysis_file and entity_excel_file:
        print(f"\n分析结果已保存到以下文件:")
        print(f"- 实体摘要: {summary_file}")
        print(f"- 分析报告: {analysis_file}")
        print(f"- 实体Excel文件: {entity_excel_file}")
    else:
        print("\n保存分析结果失败")
        print(f"- 实体摘要: {summary_file}")
        print(f"- 分析报告: {analysis_file}")
