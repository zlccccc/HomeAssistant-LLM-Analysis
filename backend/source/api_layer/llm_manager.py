...
class LLMManager:
    ...
    def __init__(self, auto_initialize: bool = True):
        ...
        Args:
            auto_initialize: 是否自动初始化ChatOpenAI模型，默认为True
        """
    ...
    def _initialize_chat_model(self):
        ...
        try:
            ...
            logger.error(f"LLMManager初始化失败: {e!s}")
            return None
    ...
    def call_openai_api(
        ...
        try:
            ...
            if (
                not self.llm
                or self.llm.temperature != temperature
                or self.llm.max_tokens != max_tokens
            ):
                try:
                    self.llm = ChatOpenAI(
                        model=self.model_name,
                    ...
                    logger.info(
                        f"重新初始化ChatOpenAI模型，temperature={temperature}, max_tokens={max_tokens}"
                    )
                except Exception as e:
                    logger.error(f"重新初始化ChatOpenAI模型失败: {e!s}")
                    return f"重新初始化模型失败: {e!s}"
            ...
            error_msg = f"调用OpenAI API失败: {e!s}"
...
class _LLMManagerProxy:
    """代理类，用于延迟初始化llm_manager"""

    def __getattr__(self, name: str) -> Any:
        return getattr(get_llm_manager(), name)

    def __setattr__(self, name: str, value: Any) -> None:
        setattr(get_llm_manager(), name, value)


llm_manager = _LLMManagerProxy()  # type: ignore[misc]
